...
class IM2SDF(data.Dataset):
    def __init__(self, config, status):
        ...
        self.grass_80d_dict = self.get_grass_80d_dict(config.grass_80d_dir)
        ...
        for cat_id in self.catlist:
            ...
            filename = './data/DISN_split/' + cat_id + '_' + status + '.lst'
            # filename = './data/DISN_split/' + cat_id + '_' + status + '_' + 'less' + '.lst'
            shape_ids = self.read_shape_ids_from_file(filename)
    ...
    def get_80d_grass(self, index):
        data = self.datalist[index]
        cat_id, shape_id = data['cat_id'], data['shape_id']
        return  self.grass_80d_dict[shape_id]
    def __getitem__(self, index):
        data = self.datalist[index]
        cat_id, shape_id = data['cat_id'], data['shape_id']
        rgb_fn = data['rgba_dir']
        h5_fn = data['h5_fn']
        grass_data_80d = self.grass_80d_dict[shape_id]
        ...
        batch_dict['grass_data_80d'] = torch.tensor(grass_data_80d).float()
        return batch_dict
    ...
    def get_testdata(self, cat_id, shape_id, cam_id):
        ...
        grass_data_80d = self.grass_80d_dict[shape_id]
        grass_data_80d = torch.tensor(grass_data_80d)
        # rgb_image = self.read_rgba_image(rgb_fn, cam_id)
        rgb_image = Image.open(
            rgb_fn + str(cam_id).zfill(2) + '.png').convert('RGB')
        rgb_real_image = rgb_image
        ...
        rgb_image = T.Normalize((0, )*3, (1, )*3)(T.ToTensor()(rgb_image))
        pc = torch.tensor(pc).float()
        batch = {'rgb_image': rgb_image.unsqueeze(0), 'gt_mesh': gt_mesh, 'grass_data_80d': grass_data_80d}
    ...
    def read_rgba_image(self, img_dir, cam_id):
        ...
        image = image/255.0
        return image
    def get_grass_80d_dict(self, grass_80d_dir):
        grass_80d_dict = {}
        # open the h5 file
        h5_f = h5py.File(grass_80d_dir, 'r')
        shape_id = h5_f["shape_id"][:].astype("<U37")
        shape_80d = h5_f["shape_80d"][:]

        for i in range(len(shape_id)):
            grass_80d_dict[shape_id[i]] = shape_80d[i]
        return grass_80d_dict
