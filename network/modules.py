...
class PerceptualPooling(nn.Module):
    ...
    def forward(self, img_featuremaps, pc, trans_mat):
        ...
        out = torch.cat((outf1, outf2, outf3, outf4), dim=1)
        return out
...
class extract_grass_80d_features(nn.Module):
    def __init__(self):
        super(extract_grass_80d_features, self).__init__()
        self.grass_80d_features_dim = 512
        self.l1 = nn.Linear(80, 256)
        self.actv = nn.ReLU()
        self.l2 = nn.Linear(256, self.grass_80d_features_dim)

    def forward(self, x):
        y = self.actv(self.l1(x))
        y = self.l2(y)
        return y


class extract_point_features(nn.Module):
    def __init__(self):
        super(extract_point_features, self).__init__()
        self.point_feature_dim = 512
        self.l1 = nn.Linear(3, 128)
        self.actv = nn.ReLU()
        # self.l2 = nn.Linear(64, 256)
        self.l3 = nn.Linear(128, self.point_feature_dim)

    def forward(self, x):
        y = self.actv(self.l1(x))
        # y = self.actv(self.l2(y))
        y = self.l3(y)
        return y

class point_80d_decoder(nn.Module):
    def __init__(self):
        super(point_80d_decoder, self).__init__()
        self.point_feature_dim = 512
        self.grass_80d_features_dim = 512
        self.l1 = nn.Linear(self.point_feature_dim + self.grass_80d_features_dim, 256)
        self.actv = nn.ReLU()
    ...
    def forward(self, x):
        ...
        y = self.l3(y)
        return y
class vox_point_decoder(nn.Module):
    def __init__(self):
        super(vox_point_decoder, self).__init__()
        self.point_feature_dim = 512
        self.vox_feature_dim = 2583
        self.l1 = nn.Linear(self.point_feature_dim + self.vox_feature_dim, 512)
        self.actv = nn.ReLU()
    ...
    def forward(self, x):
        y = self.actv(self.l1(x))
        # y = self.actv(self.l2(y))
        y = self.actv(self.l3(y))
        y = self.l4(y)
        return y
class loc_point_decoder(nn.Module):
    def __init__(self):
        super(loc_point_decoder, self).__init__()
        self.point_feature_dim = 512
        self.local_feature_dim = 1856
        # self.local_feature_dim = 960 #局部特只用了4层
        self.l1 = nn.Linear(self.point_feature_dim + self.local_feature_dim, 512)
        self.actv = nn.ReLU()
        self.l2 = nn.Linear(512, 256)
        self.l3 = nn.Linear(256, 1)
...
class VoxelDecoder2(VoxelDecoder):
    ...
    def __init__(self, feature_size, h_dim):
        ...
        self.extract_point_features = extract_point_features()
        self.extract_grass_80d_features = extract_grass_80d_features()
        self.point_80d_decoder = point_80d_decoder()
        self.vox_point_decoder = vox_point_decoder()
        self.loc_point_decoder = loc_point_decoder()


    def forward(self, p, feat, percep_feat, feat_g22, shape_80d):
        self.displacments = self.displacments.to(p.device)
        p_features = self.extract_point_features(p) ###
        p_features = p_features.transpose(1, -1) ###
        p = p.unsqueeze(1).unsqueeze(1)
        p = torch.cat([p + d for d in self.displacments], dim=2)
        ...
        shape_80d = self.extract_grass_80d_features(shape_80d)
        shape_80d = shape_80d.squeeze(1).unsqueeze(2).expand(-1, -1, p_features.shape[2])

        feat_vox_point = torch.cat((features, p_features), dim=1).permute(0, 2, 1)
        out1 = self.vox_point_decoder(feat_vox_point).permute(0, 2, 1)
        feat_loc_point = torch.cat((p_features, percep_feat), dim=1).permute(0, 2, 1)
        out2 = self.loc_point_decoder(feat_loc_point).permute(0, 2, 1)
        feat_point_80d = torch.cat((p_features, shape_80d), dim=1).permute(0, 2, 1)
        out3 = self.point_80d_decoder(feat_point_80d).permute(0, 2, 1)
        out = out1 + out2 + out3
        out = out.squeeze(1)
        return out
...
class ResEncoder(nn.Module):
    ...
    def forward(self, input_view):
        ...
        featmap_list = [feat0, feat1, feat2, feat3]
...
class ResEncoder_new(nn.Module):
    def __init__(self):
        super(ResEncoder_new, self).__init__()
