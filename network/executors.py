...
class LIHS():
    ...
    def calc_loss(self, pred, gt):
        ...
        sdf_loss = self.loss_sdf(sdf_pred, sdf_gt)
    ...
    def train(self, batch, calc_loss=True):
        ...
        sdf_gt = batch['values']
        occ_gt = batch['occ']
        grass_data_80d = batch['grass_data_80d']
        ...
        if (self.use_cuda):
            ...
            sdf_gt = sdf_gt.cuda()
            occ_gt = occ_gt.cuda()
            grass_data_80d = grass_data_80d.cuda()
            if transmat is not None:
                transmat = transmat.cuda()

        pred = self.model(img, points, grass_data_80d, transmat)
        if calc_loss:
            gt = [occ_gt, sdf_gt]
            batch_loss = self.calc_loss(pred, gt)
    ...
    def test(self, batch, eval_pred=False):
        ...
        grass_data_80d = grass_data_80d.to(self.device)
        ...
        for points_batch in self.grid_split:
            ...
            pred = self.model.module.sdf_decoder(
                points_batch, feat_vox, feat_perceptual, grass_data_80d)
    ...
    def save(self, batch, pred, fname):
        ...
        # rgb_real_image = batch['rgb_real_image']
        ...
        # torch.save(rgb_real_image, fname+'.png')
        # cv2.imwrite(fname+'.png',rgb_real_image)
